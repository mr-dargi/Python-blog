...
class CreateComment(CanCreateMixin, CommentCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if user_email:
            email = EmailMessage(
                "دیدگاه دریافت شد",
                "دیدگاه شما دریافت شد و به زودی به آن پاسخ داده می شود.",
                to=[user_email]
            )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                "دیدگاه دریافت شد",
                "پاسخ به دیدگاه شما در مقاله <<{}>> ثبت شده است برای مشاهده بر روی لینک زیر کلیک کنید:\n{}{}".format(article, current_site, reverse("blog:detail", kwargs={"slug": article.slug})),
                to=[parent_email]
            )
            email.send()
